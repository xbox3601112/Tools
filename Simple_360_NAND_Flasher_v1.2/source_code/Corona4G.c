...
int getflashsz(void)
{
	...
	{
		printf("ERROR: Unable to mount Flash!");
		return 0;
	}
	if (fopen_s(&fd, "Flash:", "rb") != 0)
	{
		printf("ERROR: Unable to open Flash: for read\n");
		fclose(fd);
		return 0;
	}
...
void try_rawdump4g(char* filename)
{
	...
	if (getflashsz() == 0)
	{
		printf("ERROR: Unable to get size of flash!\n");
		return;
	}
	...
	{
		printf("ERROR: Unable to open Flash for reading\n");
		return;
	}
	...
	if(outf == INVALID_HANDLE_VALUE)
	{
		...
		for(i = 0; i < TTL_REPS; i++)
		{
			...
			if(i%0x20) // only update this every 32 buffer fills to speed things along...
				printf("\rProcessed 0x%08x of 0x%08x bytes (%3.2f %%)", totalBytes, flash_sz, (((float)totalBytes / (float)flash_sz) * 100));
		}
		printf("\r");
		CloseHandle(outf);
		CloseHandle(flash);
...
void try_rawflash4g(char* filename)
{
	...
	printf(" * Reading in Image to Memory...\n");
	data = FileToBuffer(filename, &filesize);
	if (data == NULL)
		return;
	if (flash_sz < filesize)
	{
		printf("ERROR: %s is bigger then Flash! Aborting!\n", filename);
		free(data);
		return;
	}
	if (flash_sz > filesize)
	{
		printf("ERROR: %s is smaller then Flash! Aborting!\n", filename);
		free(data);
		return;
	}
	printf(" * Writing image to Flash...\n");
	if (Corona4GWrite(data))
		printf("Successfully wrote 0x%X bytes from %s to Flash!\n", filesize, filename);
	else
	printf("ERROR: Failed to write 0x%X bytes from %s to Flash!\n", filesize, filename);
}
...
BOOL Corona4GWrite(PBYTE data)
{
	...
	{
		printf("ERROR: Unable to open Flash for write! Aborting!\n");
		return FALSE;
	}
	lOffset.QuadPart = 0;
	error = FALSE;
	while (len > 0x4000)
	{
		...
		if (sta < 0)
		{
			printf("\rERROR: Unable to write 0x%X bytes to flash, ERROR: 0x%X Offset: 0x%X", 0x4000, GetLastError(), lOffset.LowPart);		
			error = TRUE;
		}
		else
			bWrote = 0x4000;
		...
		if ((lOffset.LowPart%(0x4000*32)) == 0)
			printf("\rProcessed 0x%X bytes of 0x%X (%3.2f %%)", lOffset.LowPart, flash_sz, (((float)lOffset.LowPart / (float)flash_sz) * 100));
	}
	printf("\r");
	if (len > 0)
	{
		...
		if (sta < 0)
		{
			printf("ERROR: Unable to write 0x%X bytes to flash, ERROR: 0x%X Offset: 0x%X", len, GetLastError(), lOffset.LowPart);		
			error = TRUE;
		}
		else
			bWrote = len;
...
PBYTE FileToBuffer(char * file, PDWORD filesize)
{
	...
	{
		printf("ERROR: Unable to open %s for reading, error: 0x%X\n", file, GetLastError());
		return dest;
	}
	...
	if (dest == NULL)
	{
		printf("ERROR: Unable to allocate 0x%X bytes buffer for %s\n", lSize, file);
		CloseHandle(inf);
		return dest;
	}
	...
	if (bRead != lSize)
	{
		printf("ERROR: Unable to read 0x%X bytes from %s (read: 0x%X bytes)\n", lSize, file, bRead);
		CloseHandle(inf);
		return NULL;
	}
	CloseHandle(inf);
